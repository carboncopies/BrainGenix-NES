...
//! Constructors
BSAlignedNC::BSAlignedNC(int _ID) {
    this->ID = _ID;
};
...
std::vector<std::shared_ptr<BSNeuron>> BSAlignedNC::GetNeuronsByID(
...
void BSAlignedNC::AttachDirectStim(std::vector<std::tuple<float, int>> ListOfStims) {
    return;
};

//! Set parameters of the distribution of the spontaneous activity
//! in the neurons in the neural circuit.
void BSAlignedNC::SetSpontaneousActivity(std::vector<std::tuple<float, float, int>>) {
    return;
};
