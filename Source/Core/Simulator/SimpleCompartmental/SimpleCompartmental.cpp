#include <Simulator/SimpleCompartmental/SimpleCompartmental.h>
...
//! Constructors
SimpleCompartmental::SimpleCompartmental(int ID, std::unordered_map<std::string, Geometries::Geometry*> & morphology) {
    this->ID = ID;
    // *** Add more here...
}

SimpleCompartmental::SimpleCompartmental(const CoreStructs::SCNeuronStruct & scneuronstruct) {
...
Geometries::Vec3D &SimpleCompartmental::GetCellCenter() {
    // *** FIX!
...
void SimpleCompartmental::AttachDirectStim(float t_ms) {
    assert(t_ms >= 0.0);
...
void SimpleCompartmental::SetSpontaneousActivity(float mean, float stdev, int Seed) {
...
void SimpleCompartmental::Record(float t_ms) {
    assert(t_ms >= 0.0);
...
CoreStructs::NeuronRecording SimpleCompartmental::GetRecording() {
    std::unordered_map<std::string, std::vector<float>> recording;
    recording["Vm_mV"] = this->VmRecorded_mV;
    return recording;
};

nlohmann::json SimpleCompartmental::GetRecordingJSON() const {
...
bool SimpleCompartmental::HasSpiked() {
...
float SimpleCompartmental::DtAct_ms(float t_ms) {
    assert(t_ms >= 0.0);
    if (this->_has_spiked) {
...
// bool SimpleCompartmental::InAbsRef(float dtAct_ms) {
...
//! Updates V_spike_t.
float SimpleCompartmental::VSpikeT_mV(float t_ms) {
    assert(t_ms >= 0.0);
...
//! Updates V_AHP_t.
float SimpleCompartmental::VAHPT_mV(float t_ms) {
    assert(t_ms >= 0.0);
 ...
 * connection.
 */
float SimpleCompartmental::VPSPT_mV(float t_ms) {
    assert(t_ms >= 0.0);
    float vPSPt_mV = 0.0;
...
void SimpleCompartmental::UpdateVm(float t_ms, bool recording) {
    assert(t_ms >= 0.0);
...
void SimpleCompartmental::DetectThreshold(float t_ms) {
    assert(t_ms >= 0.0);
...
void SimpleCompartmental::SpontaneousActivity(float t_ms) {
    assert(t_ms >= 0.0);
...
void SimpleCompartmental::Update(float t_ms, bool recording) {
    assert(t_ms >= 0.0);
...
void SimpleCompartmental::SetFIFO(float FIFO_ms, float FIFO_dt_ms, size_t reversed_kernel_size) {
...
void SimpleCompartmental::UpdateConvolvedFIFO(const std::vector<float> & reversed_kernel) {
...
void SimpleCompartmental::InputReceptorAdded(CoreStructs::ReceptorData RData) {
