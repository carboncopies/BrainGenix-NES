...
#include <Simulator/Structs/SignalFunction.h>


/**
 * @brief Test class for unit tests for functions in the namespace SignalFunctions.
 *
 */
...
TEST_F( SignalFunctionsTest, test_DoubleExponentExpr_default ) {
    ...
    tDiff = -0.5;
    ASSERT_NEAR(DoubleExponentExpr(amp, tauRise, tauDecay, tDiff),
                        0.0, tol);
...
TEST_F( SignalFunctionsTest, test_Convolve1D_default ) {
    std::vector<float> signal = {0.1, 2.0, 1.2, 2.3, 0.5, 0.7, 3.0, 0.2, 0.9, 2.1};
    std::vector<float> kernel = {1.0, 0.5, -1.0, -1.5};
    std::vector<float> expected = {0.1, 2.05, 2.1, 0.75, -2.55, -3.15, 0.6, 0.25, -3.05, -2.15, -0.15, -3.45, -3.15};
