...
bool Metal_FillSphere(BG::NES::Simulator::VoxelArray* array,
    ...
    @autoreleasepool {
        ...
        // A shared Metal buffer isolates the accelerator's storage requirements from
        // the platform-neutral VoxelArray allocator.
        size_t byteLen = (size_t)total * sizeof(VoxelType);
        id<MTLBuffer> voxelBuf = [gDevice newBufferWithBytes:rawPtr
                                                       length:byteLen
        ...
        if (success) {
            std::memcpy(rawPtr, [voxelBuf contents], byteLen);
        }
    }
