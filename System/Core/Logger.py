...
class SysLog(): # Logger Class #
    ...
    def Log(self, Message:str, Level:int=0): # Handles The Log Of An Item #
        ...
        if self.DatabaseWorking == False:
            ...
            insertStatement= ("INSERT INTO log(LogLevel,LogDatetime,CallingModule,FunctionName,LogOutput,Node) VALUES (%d, \"CAST(%s AS DATETIME)\", \"%s\", \"%s\", \"%s\", \"%s\")" %  (int(Level), LogTime, CallingModuleName.split("/")[-1].split(".")[0], CallingFunctionName, Message, self.NodeID))
